...
User’s delivery location (optional).
...
Total item storage capacity.

warehouse_location

TEXT

Store’s warehouse location.
...
Maximum wait time allowed for the order.

status

TINYINT

Request status (0 default).

cluster_id

INT

Grouping of requests (-1 default).

time_stamp

TIMESTAMP

Request creation time (auto).

delivery_price

DECIMAL

Total delivery price for the order.
...
The request that contains the item.
